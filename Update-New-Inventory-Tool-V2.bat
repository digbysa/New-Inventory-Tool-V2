...
rem   Update-New-Inventory-Tool-V2.bat --log
rem   Update-New-Inventory-Tool-V2.bat --nolog
if /I "%~1"=="--nolog" set "ENABLE_LOG=0"
if /I "%~1"=="/nolog"  set "ENABLE_LOG=0"
if /I "%~1"=="--log"   set "ENABLE_LOG=1"
if /I "%~1"=="/log"    set "ENABLE_LOG=1"

set "LOG=%USERPROFILE%\Desktop\Update-New-Inventory-Tool-V2.log"
set "PS1=%TEMP%\Update-New-Inventory-Tool-V2.ps1"
...
>> "%PS1%" echo
>> "%PS1%" echo $LogPath = Join-Path $env:USERPROFILE "Desktop\Update-New-Inventory-Tool-V2.log"
>> "%PS1%" echo
>> "%PS1%" echo $RepoOwner        = "digbysa"
>> "%PS1%" echo $RepoName         = "New-Inventory-Tool-V2"
>> "%PS1%" echo $Branch           = "main"
>> "%PS1%" echo $TargetFolderName = "New-Inventory-Tool-V2"
>> "%PS1%" echo
...
>> "%PS1%" echo         ^} else ^{
>> "%PS1%" echo             Write-Host "WARNING: Without a full backup, only Data\\**\\LocationMaster*.csv and Output contents will be preserved." -ForegroundColor Yellow
>> "%PS1%" echo             Write-Host "Other custom or modified files in the existing installation will be removed." -ForegroundColor Yellow
...
>> "%PS1%" echo
>> "%PS1%" echo     # Development-only content must not be installed on user machines.
>> "%PS1%" echo     foreach ($excludedFolder in @("Old Tool Use as refrence", "tests")) ^{
>> "%PS1%" echo         $excludedPath = Join-Path $sourcePath $excludedFolder
>> "%PS1%" echo         if (Test-Path $excludedPath) { Remove-Item -Path $excludedPath -Recurse -Force }
>> "%PS1%" echo     ^}
>> "%PS1%" echo
...
>> "%PS1%" echo     ^}
>> "%PS1%" echo
>> "%PS1%" echo     $toolBat = Join-Path $TargetPath "Launch-NewAssetTool-matching-v14.bat"
>> "%PS1%" echo     if (Test-Path $toolBat) ^{
>> "%PS1%" echo         $shortcutPath = Join-Path $Desktop "New Inventory Tool V2.lnk"
...
>> "%PS1%" echo     ^} else ^{
>> "%PS1%" echo         Write-Host "Note: Launch-NewAssetTool-matching-v14.bat not found (shortcut not created)." -ForegroundColor Yellow
>> "%PS1%" echo     ^}
>> "%PS1%" echo
...
>> "%PS1%" echo
>> "%PS1%" echo     Write-Host ""
>> "%PS1%" echo     Write-Host "Done! Updated New-Inventory-Tool-V2 on your Desktop." -ForegroundColor Green
>> "%PS1%" echo ^}
>> "%PS1%" echo catch ^{
>> "%PS1%" echo     Write-Host ""
